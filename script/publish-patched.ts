...
if (sdkExists) {
  ...
  for (const [key, value] of Object.entries(sdkPkg.exports as Record<string, string | Record<string, string>>)) {
    if (typeof value === "string") {
      const file = value.replace("./src/", "./dist/").replace(".ts", "")
      ;(sdkPkg.exports as any)[key] = {
        import: file + ".js",
        types: file + ".d.ts",
      }
    } else {
      const src = value.default ?? value.import ?? ""
      const file = src.replace("./src/", "./dist/").replace(".ts", "")
      ;(sdkPkg.exports as any)[key] = {
        import: file + ".js",
        types: value.types || file + ".d.ts",
      }
    }
  }
...
if (pluginExists) {
  ...
  // Update exports to point to dist
  for (const [key, value] of Object.entries(pluginPkg.exports as Record<string, string | Record<string, string>>)) {
    if (typeof value === "string") {
      const file = value.replace("./src/", "./dist/").replace(".ts", "")
      ;(pluginPkg.exports as any)[key] = {
        import: file + ".js",
        types: file + ".d.ts",
      }
    } else {
