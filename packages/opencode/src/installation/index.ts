...
export namespace Installation {
  ...
  export async function method() {
    ...
    for (const check of checks) {
      ...
      const installedName =
        check.name === "brew" || check.name === "choco" || check.name === "scoop" ? "opencode" : "@ex-machina/opencode"
  ...
  export async function upgrade(method: Method, target: string) {
    ...
    switch (method) {
      case "curl":
        ...
        break
      case "npm":
        result = await Process.run(["npm", "install", "-g", `@ex-machina/opencode@${target}`], { nothrow: true })
        break
      case "pnpm":
        result = await Process.run(["pnpm", "install", "-g", `@ex-machina/opencode@${target}`], { nothrow: true })
        break
      case "bun":
        result = await Process.run(["bun", "install", "-g", `@ex-machina/opencode@${target}`], { nothrow: true })
        break
      case "brew": {
