      ...
      Effect.gen(function* () {
        ...
        const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
          ...
          if (detectedMethod === "npm" || detectedMethod === "bun" || detectedMethod === "pnpm") {
            ...
            const response = yield* httpOk.execute(
              HttpClientRequest.get(`${registry}/@ex-machina/opencode/${channel}`).pipe(HttpClientRequest.acceptJson),
            )
        ...
        const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
          ...
          switch (m) {
            case "curl":
              ...
              break
            case "npm":
              result = yield* run(["npm", "install", "-g", `@ex-machina/opencode@${target}`])
              break
            case "pnpm":
              result = yield* run(["pnpm", "install", "-g", `@ex-machina/opencode@${target}`])
              break
            case "bun":
              result = yield* run(["bun", "install", "-g", `@ex-machina/opencode@${target}`])
              break
            case "brew": {
              const formula = yield* getBrewFormula()
